...
function buildClientEmail(booking) {
  ...
  const when = formatDisplayDateTime(booking.date, booking.time);
  const eventLabel = `${CLASS_TITLE} – J.B Konga by Kitti`;
  ...
  const bodyHtml = `
    ...
    <a href="mailto:${escapeHtml(INSTRUCTOR_EMAIL)}" style="color:#0b74de">${escapeHtml(INSTRUCTOR_EMAIL)}</a>
  `;
  ...
  return {
    ...
    html: emailShell({
      ...
      bodyHtml,
      footerHtml: `Üdvözlettel,<br><strong>J.B Konga by Kitti</strong><br>${escapeHtml(INSTRUCTOR_EMAIL)}`,
    }),
    text: [
      `Kedves ${booking.name}!`,
      "",
      ...
      `Google Naptár: ${calendarUrl}`,
      "",
      `Lemondás: legalább 4 órával előtte – ${INSTRUCTOR_EMAIL}`,
    ].join("\n"),
  };
}
...
async function sendBookingEmails(booking) {
  ...
  const fromAddress = SMTP.user || STUDIO_EMAIL;

  await Promise.all([
    transport.sendMail({
      from: `"J.B Konga by Kitti" <${fromAddress}>`,
      to: booking.email,
    ...
    transport.sendMail({
      from: `"J.B Konga foglalás" <${fromAddress}>`,
      to: INSTRUCTOR_EMAIL,
