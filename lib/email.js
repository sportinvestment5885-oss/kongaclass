...
let oauthTransporter;
let oauthTransporterKey = "";

async function getOAuthTransporter() {
  const oauth = getGmailOAuthConfig();
  if (!oauth.configured) {
    console.warn(
      `[email] Gmail OAuth missing (user=${oauth.user || "MISSING"}, clientId=${oauth.clientId ? "ok" : "MISSING"}, clientSecret=${oauth.clientSecret ? "ok" : "MISSING"}, refreshToken=${oauth.refreshToken ? "ok" : "MISSING"})`
    );
    return null;
  }

  const key = `${oauth.user}|${oauth.clientId}|${oauth.refreshToken.slice(0, 8)}`;
  if (oauthTransporter && oauthTransporterKey === key) {
  ...
  try {
    ...
    throw new Error(
      `Gmail OAuth token refresh failed (${code}). Ensure GMAIL_CLIENT_ID/SECRET match the credentials used to create GMAIL_REFRESH_TOKEN in OAuth Playground.`
    );
  }
  ...
  console.log(`[email] Gmail OAuth transport ready user=${oauth.user}`);
...
async function sendBookingEmails(booking) {
  ...
  console.log(
    `[email] sending booking #${booking.id} via=${provider} → client=${booking.email}, instructor=${instructorEmail}`
  );

  if (provider === "none") {
    console.warn(
      "[email] no provider configured (set GMAIL_CLIENT_ID / GMAIL_CLIENT_SECRET / GMAIL_REFRESH_TOKEN / GMAIL_USER)"
    );
  ...
  try {
    if (provider === "resend") {
      ...
      console.log(`[email] sent ok booking #${booking.id} via=resend`);
      return { sent: true, provider: "resend" };
    }

    // Gmail OAuth 2.0 (Nodemailer + googleapis) — works on Railway (HTTPS token refresh)
    const oauth = getGmailOAuthConfig();
    const transport = await getOAuthTransporter();
    if (!transport) {
      return { sent: false, reason: "oauth_not_configured" };
    }

    const fromAddress = oauth.user || getStudioEmail();
    const icsAttachment = {
      filename: "konga-edzes.ics",
      content: ics,
    ...
    console.log(
      `[email] sent ok booking #${booking.id} via=gmail_oauth clientMsg=${clientResult.messageId} instructorMsg=${instructorResult.messageId}`
    );
    return { sent: true, provider: "gmail_oauth" };
  } catch (err) {
    console.error(`[email] send failed booking #${booking.id}:`, err.message);
    if (err.message === "unauthorized_client" || err?.response?.data?.error === "unauthorized_client") {
      console.error(
        "[email] OAuth unauthorized_client: refresh token was not issued for this GMAIL_CLIENT_ID/SECRET. Re-create token in OAuth Playground with YOUR credentials (gear icon → Use your own OAuth credentials), scope https://mail.google.com/, then update GMAIL_REFRESH_TOKEN. Also revoke the old token in Google Account → Security."
      );
    }
    // Force refresh of access token on next attempt
    oauthTransporter = null;
    oauthTransporterKey = "";
    throw err;
  }
}
