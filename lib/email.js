...
async function sendBookingEmails(booking) {
  ...
  if (provider === "none") {
    console.warn(
      "[email] no provider configured (set RESEND_API_KEY for Railway, or SMTP_* for local)"
    );
  ...
  try {
    ...
    console.log(
      `[email] sent ok booking #${booking.id} via=smtp clientMsg=${clientResult.messageId} instructorMsg=${instructorResult.messageId}`
    );
    ...
    if (err.code === "ETIMEDOUT" || /timeout/i.test(err.message || "")) {
      console.error(
        "[email] SMTP blocked/timeout (common on Railway). Set RESEND_API_KEY and EMAIL_FROM instead."
      );
    }
    throw err;
  }
